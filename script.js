...
async function ShowPopup(show) {
    let showURL = (attriute == 1) ? `https://api.themoviedb.org/3/movie/${show.id}/videos?api_key=e07429ff1ac4352ccfaf1cd8f99d1ef9` : `https://api.themoviedb.org/3/tv/${show.id}/videos?api_key=e07429ff1ac4352ccfaf1cd8f99d1ef9`;
    let popElement = document.createElement("div");
    popElement.className = "ShowPopup";

    let data = await getYouTubeLink(showURL);
        ...
        setTimeout(() => {
            popElement.innerHTML = `
                ...
                <div class="PopText">
                    ...
                    <p><span>Realease Date : ${show.hasOwnProperty("release_date") ? show.release_date : show.first_air_date}</span> | <span>Avrage Ratting : ${show.vote_average} / 10</span></p>
                </div>
            `;
            ...
            document.querySelector(".backBtn").addEventListener("click", () => {
                ...
                setTimeout(() => popElement.remove(),delay);
                popupOpen = false;
            });
        },delay);
    }
...
async function getYouTubeLink(url) {
    ...
    let trailer = data.results.find(TR => TR.site === "YouTube" && TR.type === "Trailer" && TR.official === true && (TR.name.toLowerCase().includes("official trailer") || TR.name.toLowerCase().includes("series trailer")));
